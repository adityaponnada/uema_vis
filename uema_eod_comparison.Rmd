...
for (vars in var_dict){
  # print(vars[[1]])
  temp_df <- get_extreme_mapping_df(merged_df, vars[[1]], vars[[2]])
  total_observation <- nrow(temp_df)
  var_name <- strsplit(vars[[1]], 'uema_')[[1]][2]
  # print(var_name)
  # var_name <- vars[[1]]
...
extreme_mapping_df$total_perc <- extreme_mapping_df$count_list/extreme_mapping_df$total_meas_list
extreme_mapping_df$extreme_day_perc <- extreme_mapping_df$count_list/extreme_mapping_df$total_traj_list
...
Get the df for each variable with consistent high and low vals for the var and corresponding eod vals
```{r}


for (vars in var_dict){
  var_df <- get_extreme_mapping_df(merged_df, vars[[1]], var[[2]])
}
