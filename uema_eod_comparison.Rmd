...
get_var_freq_plot <- function(df, uema_var, ema_var){
  ...
  g_plot <- ggplot(var_pair_count_df, aes(x = eod_stress, y = uema_stress)) + geom_tile(aes(fill = Freq)) + scale_fill_gradient(name = 'Frequency', low = 'white', high = 'black') + theme(axis.title.y = element_blank())
  return(g_plot)
}
