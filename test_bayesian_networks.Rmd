...
## Try the test example from the package - bnlearn
Contains dummy test code to test the library
...
## Use the adjacency matrix representation of the arc set
```{r}
...
0L, 0L, 0L, 0L, 0L, 1L,
0L, 0L, 0L, 0L, 0L, 0L),
byrow = TRUE, nrow = 6, ncol = 6, dimnames = list(nodes(survey.dag), nodes(survey.dag)))


```

## Use the formula representation for the Bayesian Network
```{r}
...
Set of nodes close to a target node
```{r}
...
test_graph = graphviz.plot(survey.dag)

renderGraph(test_graph)

```

## Creating a discrete BN
```{r}
