...
var_profile_df <- data.frame()

for (i in 1:length(wake_period_list)){
  
  var_profile_df$p_id[1] <- subset_pid_df$Participant_ID[1]
  var_profile_df$wake_period[1] <- wake_period_list[[i]][1]
  for (j in 1:length(var_list)){
    var_profile_df[, ncol(var_profile_df) + 1][i] <- get_var_profile(subset_pid_df, var_list[[j]][1], wake_period_list[[i]][1])
    names(var_profile_df[ncol(var_profile_df)]) <- var_list[[j]][1]
  }
